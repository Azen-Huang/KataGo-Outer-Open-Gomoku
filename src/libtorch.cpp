...
void NeuralNetwork::infer() {
  ...
  torch::Tensor p_batch = result->elements()[0]
                              ...
                              .to(at::kCPU);
  torch::Tensor v_batch =result->elements()[2]
                              .toTensor()
                              .exp()
                              .toType(torch::kFloat32)
                              .to(at::kCPU);
  ...
  for (unsigned int i = 0; i < promises.size(); i++) {
    ...
    std::vector<double> values(static_cast<float*>(v.data_ptr()),
                             static_cast<float*>(v.data_ptr()) + v.size(0));  

    // std::cout << values[0] << " " << values[1] << " " << values[2] << std::endl;
    double win_rate = -1.0 * values[0] + values[1] * 0 + values[2];

    std::vector<double> value{win_rate};
