...
class Leaner():
    ...
    def learn(self):
        ...
        if path.exists(path.join('models', 'checkpoint.pt')):
            ...
            self.nnet.save_model('models', "checkpoint0")

        for itr in range(self.start_idx, self.num_iters + 1):
            self.num_explore = min(5 + itr // 25 * 2, self.max_num_explore)
            print("ITER :: {}".format(itr))
            print("num explore: ", self.num_explore)
            ...
            # progress = tqdm(total=self.num_eps)
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.num_train_threads) as executor:
                ...
                for k, f in enumerate(futures):
                    ...
                    print("EPS: {}, EXAMPLES: {}, total EXAMPLES: {}".format(k + 1, len(examples) // 8, len(itr_examples)))
                    # progress.update(1)

            del libtorch
            # release gpu memory
            print("Avg Game size: {}".format(len(itr_examples) // 8  / self.num_eps))
            self.num_eps = int(self.max_buffer_size / (len(itr_examples) / (self.num_eps)) + 5)
            end_time = time.time()
            execution_time = end_time - start_time
            
            print("Self play end. total example: ", len(itr_examples))
            print("Self play cost time: {} s.".format(execution_time))
            print("Next Num Eps: {}".format(self.num_eps))
            
            if (len(itr_examples) > self.max_buffer_size):
                itr_examples = itr_examples[:self.max_buffer_size]
                
            # prepare train data
            self.examples_buffer.append(itr_examples)
            # #slow window
            if (itr >= 5 and itr <= 35 and itr % 2 == 0):
                self.examples_buffer.popleft()
                print('delete olderest data.(slow window)', len(self.examples_buffer))
            ...
            if itr % self.check_freq == 0:
                self.nnet.save_model('models', "checkpoint" + str(itr))
                print("Start Contest.")
                start_time = time.time()
                ...
                end_time = time.time()
                execution_time = end_time - start_time
                print("Contest play cost time: {} s.".format(execution_time))
                
                log_path = "evaluation.log"
                with open(log_path, 'a') as f:
                    f.write("%d vs %d\n" % (itr, itr - self.check_freq))
                    f.write("NEW/PREV WINS : %d / %d ; DRAWS : %d\n" % (one_won, two_won, draws))
    ...
    def self_play(self, first_color, libtorch, show):
        ...
        while True:
            episode_step += 1
            player = MCTS(libtorch, self.num_mcts_threads, self.c_puct,
                     self.num_mcts_sims, self.c_virtual_loss, self.action_size, True)
            # print("Round {}".format(episode_step))
            # get action prob
            ...
            for i in range(len(prob)):
                if legal_moves[i] == 1 and prob[i] == 0:
                    prob[i] = 2e-4
            train_prob = prob
            train_prob /= np.sum(train_prob)

            action = np.random.choice(len(prob), p=prob) if episode_step <= self.num_explore else int(np.argmax(prob))

            # generate sample
            board = tuple_2d_to_numpy_2d(gomoku.get_board())
            last_action = gomoku.get_last_move()
            cur_player = gomoku.get_current_color()
            value = list(gomoku.get_action_value())[action]

            if (value == -2.0):
                value = 0
            
            sym = self.get_symmetries(board, train_prob, last_action)
            for b, p, a in sym:
                # average value target
                # value [0, 1] 勝率
                train_examples.append([b, a, cur_player, p, value])
            ...
            if ended == 1:
                ...
                for i in range(len(train_examples)):
                    x_0 = train_examples[i][0]
                    x_1 = train_examples[i][1]
                    x_2 = train_examples[i][2]
                    x_4 = train_examples[i][4]

                    p_t = train_examples[i][3]
                    nxt_p_t = train_examples[i+8][3] if (i+8) < len(train_examples) else [float(1/225)] * 225
                    v_t = [1 - float((1.0 + float((x_2 * winner + x_4) / 2.0)) / 2.0), 0, float((1.0 + float((x_2 * winner + x_4) / 2.0)) / 2.0)]
                    if (gomoku.get_size() > 220):
                        v_t = [0.0, 1.0, 0.0]
                    
                    train_data.append((x_0, x_1, x_2, p_t, nxt_p_t, v_t))
    ...
    def contest(self, network1, network2, num_contest):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.num_train_threads // 4) as executor:
    ...
    def _contest(self, network1, network2, first_player, show):
        ...
        while True:
            ...
            if episode_step <= 9:
                ...
                prob = np.array(list(player.get_action_probs(gomoku, 0)))
            best_move = np.random.choice(len(prob), p=prob) if episode_step <= 9 else int(np.argmax(prob))
    ...
    def play_with_human(self, human_first=True, checkpoint_name="best_checkpoint"):
        ...
        mcts_best = MCTS(libtorch_best, self.num_mcts_threads * 2, \
             self.c_puct, self.num_mcts_sims * 6, self.c_virtual_loss, self.action_size, False)
